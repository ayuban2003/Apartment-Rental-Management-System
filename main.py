# This is the refactored code of APTLY. This code uses functions and logics that are modern 
# or up-to-date code and uses latest functions for better security. 
...
def AdminLogin():
    ...
    if 'loggedin' in session and session.get('username'):
        return redirect(url_for('AdminDashboard'))

    # Initialize error variable to None
    error = None

    # Check if the request method is POST and required form fields are present
    if request.method == 'POST':
        # Retrieve form data
        admin_username = request.form.get('adminUsername')
        admin_password = request.form.get('adminPass')
        security_pass = request.form.get('securityPass')

        # Proceed only if all required form fields are provided
        if admin_username and admin_password and security_pass:
            try:
                # Connect to the database and execute query to fetch admin details
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute('SELECT * FROM Admins WHERE username = %s', (admin_username,))
                account = cursor.fetchone()

                # Validate credentials if the account exists
                if account:
                    if check_password_hash(account['password_hash'], admin_password) and account['security_password'] == security_pass:
                        # Set session variables for logged-in admin
                        session['loggedin'] = True
                        session['id'] = account['id']
                        session['username'] = account['username']

                        # Flash success message and redirect to admin dashboard
                        flash('You have logged in successfully!!')
                        return redirect(url_for('AdminDashboard'))
                    else:
                        # Invalid credentials error
                        error = 'Invalid username, password, or security pass!'
                else:
            ...
            # Missing form data error
            error = 'All fields are required!'

    # Render the login template with error message (if any)
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'confirm_password' in request.form:
        ...
        if password != confirm_password:
            ...
            if account:
                ...
                security_password = ''.join(random.choices(string.ascii_letters + string.digits, k=10)) 

                try:
                    cursor.execute("INSERT INTO Admins (username, password_hash, security_password) VALUES (%s, %s, %s)", 
                                   (username, hashed_password, security_password)) 
                    mysql.connection.commit()
                    msg = 'You have successfully registered! Please note your security password: ' + security_password 
                    success = True 
                except Exception as e:
                    msg = f"Registration failed: {e}"
                    success = False 

    return render_template('Adminregister.html', msg=msg, success=success) 
...
def alreadyTenant() :
    ...
    if request.method == 'POST' and 'Username' in request.form and 'aptNo' in request.form :
        ...
        return redirect(url_for('Contract.html', aptNo=aptNo ,Tname=Tname, Uname=Uname,PhNo=PhNo , rentAmt=rentAmt, late_fee=late_fee, totAmt=totAmt))
    else :
        Error = 'Invalid Username or Apartment No.!!'
    if request.method == 'POST' :
        Error= 'Please fill out the form!'
